...
// Following macro definitions may interfere with <limits>
#ifdef max
#undef max
#endif // max
#ifdef min
    ...
    if (m_socket == INVALID_SOCKET)
    {
        ...
        if (m_socket != INVALID_SOCKET)
        {
            ...
            if (res != SOCKET_ERROR)
            {
                ...
                m_mcast_addr.sin_addr.S_un.S_addr = htonl(INADDR_ANY);
                ...
                if (res != SOCKET_ERROR)
                {
                    ...
                    ip_mreq mreq = { 0 };
                    inet_pton(AF_INET, ip_address.c_str(), &mreq.imr_multiaddr.S_un.S_addr);
